// forked from design-sync lib/source-kit.mjs — group derivation only, so the
// three components under src/components/reui/ take their doc `category` like
// every other component instead of keeping a vendor-directory group.
//
...
// `reui` is a vendored-namespace directory, not a component family — grouping
// by it would strand Cascader/Filters/DataGrid outside the doc categories.
const GENERIC_DIR = new Set(['components', 'component', 'src', 'lib', 'ui', 'packages', 'react', 'reui']);
const kebabOf = (n) => n.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase();
    ...
    for (const c of components) {
      ...
      c.group = slug(
        slash(relative(srcRoot, dirname(hit)))
          .split('/')
          // Also drop the kebab-cased form of the component's own name:
          // `reui/data-grid/data-grid.tsx` must not group DataGrid as
          // "data-grid" — the upstream check only compares the squashed name.
          .filter((s) => s && s.toLowerCase() !== c.name.toLowerCase()
            && s.toLowerCase() !== kebabOf(c.name) && !GENERIC_DIR.has(s.toLowerCase()))
          .at(-1)
