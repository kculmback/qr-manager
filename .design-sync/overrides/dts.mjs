// forked from design-sync lib/dts.mjs — shadcn-style flat subpart exports (CardHeader,
// SidebarMenuButton, …) carry no `compounds` namespace, so the bundled
// partitionSubcomponents can't fold them and every subpart becomes its own card.
//
...
// Fork state: name → declaring file, filled by loadDts (which always runs
// before partitionSubcomponents in package-build).
const fileOf = new Map();
// Directories whose name never identifies a component family.
const GENERIC_DIR = new Set(['components', 'component', 'src', 'lib', 'ui', 'react', 'reui', 'dist']);
const kebab = (n) => n.replace(/([a-z0-9])([A-Z])/g, '$1-$2').toLowerCase();
const dirOf = (fp) => fp.split('/').slice(-2, -1)[0] ?? '';
const baseOf = (fp) => (fp.split('/').pop() ?? '').replace(/\.d\.ts$/, '').replace(/\.tsx?$/, '');

export function partitionSubcomponents(names, compounds) {
  const set = new Set(names);
  const parentOf = new Map();

  // Fork: the "primary" export of a directory — the one declared in the file
  // named after its own directory (reui/cascader/cascader.d.ts → Cascader).
  // Every other export declared anywhere under that directory is one of its
  // parts. Generic container dirs (components/) have no primary, so those
  // fall through to the same-file rule below.
  const primaryOfDir = new Map();
  for (const n of names) {
    const fp = fileOf.get(n);
    if (!fp) continue;
    const dir = dirOf(fp);
    if (GENERIC_DIR.has(dir.toLowerCase())) continue;
    if (baseOf(fp) === dir && kebab(n) === dir) primaryOfDir.set(dir, n);
  }
  ...
  for (const n of names) {
    ...
    if (parentOf.has(n)) continue;
    const fp = fileOf.get(n);
    if (!fp) continue;

    // Fork rule 1 — directory primary.
    const primary = primaryOfDir.get(dirOf(fp));
    if (primary && primary !== n && set.has(primary)) { parentOf.set(n, primary); continue; }

    // Fork rule 2 — same file, PascalCase prefix. `Card` and `CardHeader` are
    // declared side by side in card.d.ts; `Button` (button.d.ts) and
    // `ButtonGroup` (button-group.d.ts) are not, so ButtonGroup stays a root.
    for (let i = parts.length - 1; i >= 1; i--) {
      const prefix = parts.slice(0, i).join('');
      if (set.has(prefix) && prefix !== n && fileOf.get(prefix) === fp) { parentOf.set(n, prefix); break; }
    }
  }
  ...
  // subs whose immediate parent is itself a sub. Cycles are impossible for
  // the prefix rules (each parent has strictly fewer PascalCase parts); the
  // directory rule can't cycle either (a primary is never given a parent by
  // it), but guard the walk anyway.
  for (const [n] of parentOf) {
    let p = parentOf.get(n), guard = 0;
    while (parentOf.has(p) && guard++ < 32) p = parentOf.get(p);
    parentOf.set(n, p);
  }
  return { parentOf };
}
...
export function loadDts(typesRoot) {
  ...
  // ── fork addition: where each export is actually declared ──────────────
  // Re-export chains resolve to the defining file, so `Filters` (filters.d.ts)
  // and `FilterChip` (filters-chip.d.ts) land in their own files even though
  // filters.d.ts re-exports both. partitionSubcomponents reads this below.
  fileOf.clear();
  if (sf) for (const [name, decls] of sf.getExportedDeclarations()) {
    if (!/^[A-Z][A-Za-z0-9]*$/.test(name)) continue;
    const d = decls.find((x) =>
      Node.isVariableDeclaration(x) || Node.isFunctionDeclaration(x) ||
      Node.isClassDeclaration(x)) ?? decls[0];
    const fp = d?.getSourceFile?.().getFilePath();
    if (fp && fp.split('\\').join('/').startsWith(pkgDir.split('\\').join('/'))) fileOf.set(name, fp);
  }
