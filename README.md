...
> In this template, we use `@qr-manager` as a placeholder for package names. As a user, you might want to replace it with your own organization or project name. You can use find-and-replace to change all the instances of `@qr-manager` to something like `@my-company` or `@project-name`.
...
pnpm --filter @qr-manager/auth generate
```
...
3. Outputs a Drizzle-compatible schema file to the `@qr-manager/db` package
