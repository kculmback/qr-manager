...
import { Tabs, TabsList, TabsTrigger } from "@qr-manager/ui/components/tabs";
...
export function EnableTwoFactorDialog({
  ...
  const {
    ...
    onError: () =>
      toast.add({
        type: "error",
        title: twoFactorLocalization.setupKeyCopyFailed,
      }),
  });
  ...
  const {
    ...
    onSuccess: (data) => {
      if (data.method === "otp") {
        toast.add({
          type: "success",
          title: twoFactorLocalization.twoFactorEnabled,
        });
